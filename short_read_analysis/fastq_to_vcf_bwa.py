...
def get_ext(fname):
    if fname.endswith('.gz'):
        ...
        base = fname
        ext = ''
    candext = os.path.splitext(base)[1]
    if candext == '.txt':
        candext = '.fastq'
    ext = candext + ext
    return ext
...
map_reads_exec = 'map_reads_by_indiv-bwastampy.py'
...
if __name__ == '__main__':
    ...
    parser.add_argument('-th','--tcp_host',default=tcp_host,type=str,help='host for DB lookup.'+ds)
    ...
    print >> sys.stderr, 'use %s for network' % tcp_host
    ...
    #index_lookup = preprocess_radtag_lane.get_table_as_dict(multiplex_idx_db)
    #td = preprocess_radtag_lane.get_table_as_dict(config.LIBRARY_DATA)
        ...
        if r1:
            ...
            if r2:
                glob_key = os.path.join(d['datapath'], \
                                        'reads_by_individual', \
                                        '%s_lane%s%s_trim' % (d['flowcell'],d['lane'],get_idx_str(d.get('index',None))), \
                                        '%s_*%s' % (ind,get_ext(r1)) )
                expected_fq_d[glob_key] = 2
                glob_key = os.path.join(d['datapath'], \
                                        'reads_by_individual', \
                                        '%s_lane%s%s_merge' % (d['flowcell'],d['lane'],get_idx_str(d.get('index',None))), \
                                        '%s_*%s' % (ind,get_ext(r1)) )
                ...
                glob_key = os.path.join(d['datapath'], \
                                        'reads_by_individual', \
                                        '%s_lane%s%s' % (d['flowcell'],d['lane'],get_idx_str(d.get('index',None))), \
                                        '%s_*%s' % (ind,get_ext(r1)) )                
