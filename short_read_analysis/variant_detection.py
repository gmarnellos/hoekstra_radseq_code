        ...
        if line.startswith('#CHROM'):
            ...
            for ind,gt in zip(headers[FORMAT+1:],fields[FORMAT+1:]):
                ...
                if ':' in gt:
                    ...
                    if this_gt.has_key('GQ') and this_gt['GQ'] != '.':
            ...
            if cutoff_fn is None or cutoff_fn(sd):
                if write_thresholded_vcf is not None:
                    ...
                    newdline = newdline.strip()
                    if newdline: ofh.write(newdline + '\n')
...
def sd_from_htseq_vc(vc):
    sd = {}
    sd['CHROM'] = vc.pos.chrom
    sd['POS'] = vc.pos.pos
    sd['REF'] = vc.ref
    sd['ALT'] = ','.join(vc.alt)
    sd.update(vc.info)
    for k,v in sd.items():
        if type(v) == list:
            sd[k] = ','.join(map(str,v))
    sd['indiv_gt'] = dict([(k,v) for k,v in vc.samples.items() if v.has_key('PL')])
    return sd

def process_htseq_vc(vc,indiv_gt_phred_cut):
    vc.samples = dict([(k,v) for k,v in vc.samples.items()
                       if float(v.get('GQ',0)) >= indiv_gt_phred_cut])
    if len(vc.samples) > 0:
        vc.info['fh'] = fract_het(vc.samples)
        vc.info['mac'] = maf(vc.samples)
    else:
        vc.info['fh'] = 0
        vc.info['mac'] = 0
    vc.info['numind'] = len(vc.samples)
    vc.info['QUAL'] = float(vc.qual)
    return vc

def minimal_sd(sd,sd_keys,indiv_gt_keys):
    minsd = {}
    for k in sd_keys:
        minsd[k] = sd.get(k,None)
    if indiv_gt_keys:
        minsd['indiv_gt'] = {}
        for ind,gtd in sd['indiv_gt'].items():
            minsd['indiv_gt'][ind] = {}
            for k in indiv_gt_keys:
                minsd['indiv_gt'][ind][k] = sd['indiv_gt'][ind][k]
    return minsd

def load_minimal_vcf_htseq(vcf_file,cut_fn=lambda sd: True,indiv_gt_phred_cut=20, \
                           multiallelic_fn=lambda sd: not ',' in sd['ALT'], \
                           sd_keys=['REF','ALT'], indiv_gt_keys=['GT','PL'],min_ind=0,min_qual=0,
                           max_sites=numpy.inf,max_lines=numpy.inf):
    import HTSeq
    vcfr = HTSeq.VCF_Reader(vcf_file)
    vcfr.make_info_dict()
    vcfr.parse_meta()
    passed = 0
    ...
    for i,vc in enumerate(vcfr):
        if len(min_vcf_obj)>=max_sites or i>max_lines:
            break
        if i % 1000 == 0:
            thistick = time.time() - tick
            print >> sys.stderr,'\r', i,len(min_vcf_obj),thistick,thistick/(i+1),
        if vc.qual < min_qual: continue
        if len([v for v in vc.samples.values() if v['GT'][0] != '.']) < min_ind: continue
        vc.unpack_info(vcfr.infodict)
        vc = process_htseq_vc(vc,indiv_gt_phred_cut)
        if not cut_fn(vc.info): continue
        passed += 1
        sd = sd_from_htseq_vc(vc)
        if len(vc.alt) > 1:
            bsd = multiallelic_fn(sd)
            if bsd:
                min_vcf_obj[(vc.pos.chrom,vc.pos.pos)] = minimal_sd(bsd,sd_keys,indiv_gt_keys)
        else:
            min_vcf_obj[(vc.pos.chrom,vc.pos.pos)] = minimal_sd(sd,sd_keys,indiv_gt_keys)
...
def biallelic_sd(sd,GQ_method='nextbest',transfer_fields=['CHROM','POS','AN','QUAL','QD','MQ0'],skip_fields=['indiv_gt']):
    ...
    delta_L = 0
    for ind,gtd in sd['indiv_gt'].items():
        if not (gtd.has_key('AD') and gtd.has_key('PL')): continue
